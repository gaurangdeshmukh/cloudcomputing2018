...
GATEWAY_ID=$(aws ec2 describe-internet-gateways \
 --query 'InternetGateways[*].{InternetGatewayId:InternetGatewayId}' --filters "Name=attachment.vpc-id,Values=$VPC_ID"  --output text)
if [ $? -ne "0" ]
then
...
while 
sub=$(aws ec2 describe-subnets --filters Name=vpc-id,Values=$VPC_ID  --query 'Subnets[*].SubnetId' --output text)
[[ ! -z $sub ]]
do
...
Con=$(aws ec2 describe-route-tables --filters Name=vpc-id,Values=$VPC_ID --query RouteTables[*].Routes[].{DestinationCidrBlock:DestinationCidrBlock} --output text| grep '0.0.0.0/0')
